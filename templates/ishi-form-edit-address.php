...
?>
<form data-ishi-ui-form method="post" data-ishi-ui-url="<?php echo esc_url( Ishi_WooCommerce_Addresses::endpoint( $load_address ) ); ?>" novalidate onsubmit="return false;">
    <h2><?php echo wp_kses_post( apply_filters( 'woocommerce_my_account_edit_address_title', $page_title, $load_address ) ); ?></h2>
    <p data-ishi-ui-unavailable role="status"><?php esc_html_e( 'Background address saving is loading or unavailable. If this message remains, enable JavaScript or contact support. No changes will be submitted.', 'ishi-latepoint-profile' ); ?></p>
    <fieldset data-ishi-ui-ready disabled>
    <div class="woocommerce-address-fields">
        ...
        <p>
            <button type="button" data-ishi-ui-save class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="ishi_save_address" value="<?php echo esc_attr( $load_address ); ?>"><?php esc_html_e( 'Save changes', 'ishi-latepoint-profile' ); ?></button>
