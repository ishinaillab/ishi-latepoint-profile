...
<p data-ishi-ui-unavailable role="status"><?php esc_html_e( 'Address editing is loading or unavailable. If this message remains, enable JavaScript or contact support.', 'ishi-latepoint-profile' ); ?></p>
<div data-ishi-ui-view class="u-columns woocommerce-Addresses col2-set addresses ishi-ui-card-grid">
    <?php $column = 0; foreach ( $get_addresses as $name => $address_title ) :
        ++$column;
        $address = wc_get_account_formatted_address( $name, $customer_id );
        ?>
        <div class="u-column<?php echo esc_attr( $column ); ?> col-<?php echo esc_attr( $column ); ?> woocommerce-Address ishi-ui-card">
            <header class="woocommerce-Address-title title ishi-ui-card-header">
                <h2><?php echo esc_html( $address_title ); ?></h2>
                <button type="button" disabled data-ishi-ui-control data-ishi-ui-url="<?php echo esc_url( Ishi_WooCommerce_Addresses::endpoint( $name ) ); ?>" class="edit" data-ishi-ui-edit>
                    <?php printf( $address ? esc_html__( 'Edit %s', 'woocommerce' ) : esc_html__( 'Add %s', 'woocommerce' ), esc_html( $address_title ) ); ?>
                </button>
            </header>
            <div class="ishi-ui-card-body">
            <address>
                <?php
